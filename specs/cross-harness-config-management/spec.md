...
- [ ] AC-25: WHEN the app or CLI starts with network access THE SYSTEM SHALL fetch versioned, signature-verified surface definitions (paths, formats, capability matrix, recommendation rules) from harnesskit.ai; WHEN offline or verification fails THE SYSTEM SHALL fall back to the release-bundled snapshot and say so. *(In progress 2026-09-08, M4: verification core merged (PR #374) and now WIRED. `resolveDefinitions` in core is shared by both apps — fetch, verify-before-parse, re-verify the cache, 24h TTL, anti-rollback against a machine-history floor, snapshot fallback with a stated reason. The CLI (`diff`, `sync`) and the desktop Machine view both resolve their surface registry through it, and schema v4 persists the cache (exact signed bytes + detached signature) and the rollback floor in separate tables. The desktop verifies via a Rust Ed25519 command, since `node:crypto` is what the webview cannot load; the transport is shared, because it uses only web standards. Key ROTATION is deliberately NOT implemented and D7 is withdrawn — see the design.md §7 amendment. REMAINING: no publisher keypair has been generated, so `PUBLISHER_KEYS` is empty and every run correctly ends on the release snapshot without touching the network; and CI bundle publishing plus hosting are not built. Custody is decided: the private key stays offline and CI publishes a pre-signed artifact. Known residual: no freshness bound.)*
