...
- [ ] AC-2: WHEN a surface is installed but a resource kind is absent THE SYSTEM SHALL distinguish "not configured" from "not detected" from "concept unsupported by this harness" (e.g. pi × MCP). *(Extended 2026-09-02, M3: a fourth state, `unmanaged`, separates "this harness has the concept but HarnessKit reads no store for it here" (cursor × plugin, windsurf × mcp-server) from "this surface could hold it and does not". The engine already excluded the former from gaps; the grid was rendering both as the same blank cell.)*
...
- [ ] AC-4: WHEN inventory runs THE SYSTEM SHALL enumerate installed plugins and registered marketplaces per surface for surfaces with a plugin model (claude-code, copilot-vscode, copilot-cli, codex, cursor). *(Scoped 2026-09-02, M3: shipped for **claude-code** (`~/.claude/plugins/installed_plugins.json` + `known_marketplaces.json`) and **codex** (`[plugins."name@marketplace"]` and `[marketplaces.ID]` in `~/.codex/config.toml`) — both formats verified against a real installation of each. The other three are DEFERRED, not dropped, because no on-disk plugin state could be verified for them: `copilot-cli` has a real plugin model (`copilot plugin list`) but writes no discoverable state until a plugin is installed and offers no machine-readable list output; `cursor-agent` advertises no plugin subcommand at all; and `copilot-vscode`'s extensions are VS Code extensions, not agent plugins. Inventing readers for unverified formats would ship silently-wrong inventory, so those three render as the `unmanaged` cell state — added in this milestone, since the grid previously collapsed it into a blank "absent" cell — until a format can be confirmed. Both shipped surfaces share the `name@marketplace` identity convention, so a plugin installed on both joins one grid row without a translation table.)*
...
- [ ] AC-9: WHEN a resource exists on at least one surface and is absent on another surface that supports the concept THE SYSTEM SHALL flag the gap and offer the sync actions of AC-11. *(Refined 2026-09-02, M3: a plugin gap is flagged only where the target has the plugin's marketplace registered. Plugin identity is `name@marketplace` and marketplaces are per-surface, so a plugin from an unregistered marketplace is not one action away, and some are unreachable outright — Codex's bundled marketplaces are local directories inside the ChatGPT app. Suppressed only where the target's marketplaces were actually read; "cannot say" never hides a gap.)*
- [ ] AC-10: WHEN a baseline profile is configured (a git-hosted harness.yaml the user extends) THE SYSTEM SHALL diff the machine against it and present missing-vs-baseline as recommendations alongside machine-gap recommendations. Recommendations come only from these two deterministic sources in v1. *(Done 2026-09-07, M3: `harness-kit status --baseline <path>`. machine-gap reads the engine's own gap list rather than recomputing, so a recommendation can never propose what the grid calls unreachable; baseline-gap reports what the profile declares and the machine has NOWHERE, since a baseline says what a machine should have, not which surface should have it. A local path only — fetching a git-hosted profile needs the signed-definitions transport from M4, and a URL is refused with that reason.)*

### Sync actions (three action surfaces)

- [ ] AC-11: WHEN the user selects any gap or diff THE SYSTEM SHALL offer all three action surfaces: (a) direct apply, (b) the equivalent `harness-kit` CLI command shown verbatim, (c) a generated agent prompt that instructs the target harness's own agent to make the change. *(Scoped 2026-09-01: M2 covers the **gap** case — a resource present on one surface and absent on another. The **diff** case — a row present on every surface whose content differs — is deferred to M3. Closing a gap is a copy; reconciling a diff is a three-way merge question the user has to answer (which side wins, per field), and shipping it as a one-click copy would silently pick a winner. It rides with the recommendation work in M3.)* *(Done 2026-09-07, M3: a plan whose target already holds the resource with different content carries `overwrites` — the per-field deltas that would be replaced — and always requires explicit confirmation. The user picks the winner, but only after being shown what is lost. Compared on canonical forms, so a skill's `skillPath` and a rotated secret are not reported as losses.)*
- [ ] AC-12: WHEN a cell's capability is native THE SYSTEM SHALL apply direct writes for the tier-one kinds at launch: `mcp-server`, `skill`, `instructions`. Other kinds MAY launch with CLI-native-command and agent-prompt surfaces only, and gain direct writes per the capability matrix. *(Amended 2026-09-01: `plugin` moves to M3 with the PluginBroker of AC-18/AC-19 — a plugin action shells out to a native installer or unpacks a manifest rather than writing a canonicalized resource, so it does not share this milestone's write path. Plugin cells render read-only with their M3 affordance until then.)* *(M3, 2026-09-02: plugin cells now render live — present/absent/diff per surface — and offer the CLI command and agent prompt (AC-13). Direct apply stays refused with a reason naming the installer as the mechanism rather than promising a later direct write; no plugin format has a writer, so no apply can reach an install record. The broker itself is the next step.)*
...
- [ ] AC-31: WHEN a direct apply targets a path outside the project root THE SYSTEM SHALL execute it through a named transaction root (`home`), keeping every member path relative within that root so the existing traversal and symlink-boundary guards apply unchanged, and SHALL reject any path the surface registry does not declare as a config store for that surface and scope. *(Tightened 2026-09-02: the allowlist derives from user-scope stores this build can WRITE, not from every store declared. Adding a read-only store — plugin enumeration — must not widen what an apply may touch. This also narrowed the existing allowlist: the four `json-generic` permissions files left it, since that format has no writer and `planStoreWrite` already refused them.)*
...
- [ ] AC-34: WHEN the capability matrix reports that the target surface cannot fully express the resource being copied THE SYSTEM SHALL present the loss report naming each dropped or downgraded field before mutating anything, and SHALL NOT apply without explicit confirmation (`--yes` constitutes confirmation in the CLI). *(Calibrated 2026-09-01: loss is always **reported**, but confirmation is required only for `source-only` / `unsupported` / `not-applicable`. A `translated` cell — the matrix's answer for e.g. cursor × mcp-server — means the resource IS expressed, just mapped into the surface's native shape, which is exactly what the writer does. Gating on translation would demand confirmation for nearly every copy and train users to click through the one prompt that matters.)*
...
- [ ] AC-37: WHEN the release ships THE SYSTEM SHALL present drift within the Machine view, redirect the legacy `/drift` route to it, and migrate existing drift acknowledgements onto the shared state store. *(Deferred 2026-09-01 to M3, in full. The premise was wrong: Drift detects `harness.yaml` ↔ compiled-output drift and offers acknowledge/fix, while the Machine grid compares surfaces against each other. They are different comparisons, so Machine does not already cover it and routing `/drift` at Machine would remove the acknowledge/fix workflow rather than absorb it. Absorption needs Machine to grow that workflow, plus the StateStore bridge for the acknowledgements.)* *(Done 2026-09-07, M3: the Machine view now renders the Drift workflow as a section — the existing page, not a reimplementation, so acknowledge and fix survive intact. `/drift` redirects to `/machine` rather than 404ing a bookmark. Acknowledgements move to the shared `harness.db` under a new additive schema v3; the migration COPIES and never deletes the source rows, so a wrong migration costs a user nothing. A missing legacy database, or one whose owner never used Drift, is zero rows rather than an error.)*
...
- [ ] AC-18: WHEN installing a plugin onto a surface with a native installer THE SYSTEM SHALL orchestrate that installer (`claude plugin install … --scope …`, `copilot plugin install …`, Codex/Cursor equivalents) rather than writing plugin files itself. *(Done 2026-09-07, M3: verbs and flags live in the registry as descriptor data, each read from the tool's own `--help`. Cursor has no plugin subcommand at all, so it has no installer to orchestrate. Two structural guards: argv never a shell, and no value argument may look like a flag — a plugin literally named `--config` would otherwise become an option to the installer.)*
- [ ] AC-19: WHEN installing a plugin onto a surface without a compatible plugin model (pi, opencode) THE SYSTEM SHALL unpack the plugin's skills/MCP/instructions into surface-native locations and track the installation in HarnessKit state so update and uninstall work. *(Partially done 2026-09-07, M3: skills unpack and every written path is recorded in `plugin_installs`. MCP servers and instructions are NOT unpacked — each needs merge semantics against a file the user also writes, and the driver refuses out loud rather than half-doing it. Uninstall likewise refuses pending a read-back of the recorded file list. The unpack writes directly rather than through the file-transaction engine, so it has no preimage check, though it now shares that engine's symlink guard. **It also has no reachable caller yet:** pi lists `plugin` as not-applicable and opencode declares no plugin store, so neither ever enters `missingOn`, and both the CLI and the drawer draw targets only from gaps. The driver, its file recording and its refusals are exercised by unit tests alone. Making it reachable needs an explicit "unpack into this surface" affordance — a deliberate action, not a gap-closing copy, since a surface with no plugin concept can never be missing a plugin.)*
...
- [ ] AC-25: WHEN the app or CLI starts with network access THE SYSTEM SHALL fetch versioned, signature-verified surface definitions (paths, formats, capability matrix, recommendation rules) from harnesskit.ai; WHEN offline or verification fails THE SYSTEM SHALL fall back to the release-bundled snapshot and say so. *(In progress 2026-09-08, M4: the verification core landed and merged (PR #374) — verify-before-parse, anti-rollback against a machine-history floor with a fail-closed guard on a corrupt floor, injected `Fetcher`/`SignatureVerifier` seams, and snapshot fallback with a stated reason. Key rotation is NOT implemented: cross-signed transition statements (D7) were built and then WITHDRAWN, because a replayed statement resurrected a key revoked via `notAfter` and revoking the outgoing key destroyed the incoming one — see the trust-anchor note in `packages/core/src/definitions/feed.ts`. The trust anchor is compiled-in keys only, so introducing a key still needs a release. NOT yet wired: nothing calls `loadDefinitions`, so no remote data reaches the registry and AC-26 is unproven. Also open: `definitions_cache` persistence (the v1 placeholder table has no signature column and stores `payload TEXT`, but re-verification needs the exact signed bytes plus the detached signature), a desktop verifier (which cannot use `node:crypto`), CI bundle publishing, hosting, and publisher key custody — which design.md §7 and ADR 0004 do not specify. Known residual: no freshness bound, which also means a frozen client never receives the `notAfter` that would retire a compromised key.)*
