...
Harness Protocol resolves four ordered layers: organization → personal → project → session. The nearest resource definition wins, while organization policy remains a ceiling that lower layers cannot widen. Native tools expose different subsets of these scopes, so unsupported scope operations remain source-only or blocked rather than being presented as native.
...
| **Session** | Ephemeral session overlay | Ephemeral session overlay | Ephemeral session overlay |

Harness Kit reconciles all four layers. Session overlays remain ephemeral unless explicitly captured or promoted. Personal and device reconciliation state stays local; organization profiles and policy can be distributed through the optional registry service.
...
Harness Kit leans into this: it reconciles portable intent with native peer files and writes idiomatic output for each harness, while reporting the gaps where native compatibility ends.
</Callout>
