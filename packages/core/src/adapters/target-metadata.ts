 ...
 * (paths, required binaries, directory layout) — a different concern from
 * the `HarnessAdapter` behavior objects in registry.ts, so it lives here
 * rather than being folded into the adapter registry itself.
 *
 * `packages/core/src/compile/targets.ts` re-exports everything below as a
 * deprecated shim so existing callers (mcp-servers.ts, check.ts, the CLI,
