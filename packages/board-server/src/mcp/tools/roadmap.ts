...
export const roadmapTools = [
  ...
  },
  {
    name: "save_roadmap",
    description:
      "Persist a roadmap for a project (Claude generates the roadmap object and calls this to save it)",
    inputSchema: {
      type: "object" as const,
      properties: {
        project: { type: "string", description: "Project slug" },
    ...
    handler: async (args: { project: string; roadmap: Record<string, unknown> }) => {
      ...
      if (missingFields.length > 0) {
        return {
          content: [
            {
              type: "text" as const,
              text: `Invalid roadmap: missing required fields: ${missingFields.join(", ")}`,
            },
          ],
        };
      }
      ...
      roadmapStore.writeRoadmap(
        args.project,
        roadmap as unknown as import("../../roadmap-types.js").Roadmap,
      );
      return {
        content: [{ type: "text" as const, text: `Roadmap saved for project "${args.project}"` }],
      };
    },
  },
  {
  ...
  },
  {
    ...
    handler: async (args: { project: string; analysis: Record<string, unknown> }) => {
      ...
      if (!Array.isArray(raw.competitors)) {
        return {
          content: [
            {
              type: "text" as const,
              text: "Invalid competitor analysis: missing required field: competitors (array)",
            },
          ],
        };
      }
      ...
      roadmapStore.writeCompetitorAnalysis(
        args.project,
        analysis as unknown as import("../../roadmap-types.js").CompetitorAnalysis,
      );
      return {
        content: [
          {
            type: "text" as const,
            text: `Competitor analysis saved for project "${args.project}"`,
          },
        ],
      };
    },
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        status: {
          type: "string",
          enum: ["backlog", "planning", "in_progress", "done"],
    ...
    handler: async (args: {
      ...
      if (features.length === 0) {
        return {
          content: [{ type: "text" as const, text: "No features match the given filters" }],
        };
      }
    ...
    inputSchema: {
      ...
      properties: {
        ...
        feature: {
          type: "object",
          description:
            "Feature object (title, description, rationale, priority, complexity, impact, phaseId, status, acceptanceCriteria, userStories are required)",
        },
      },
      ...
      if (!roadmap) {
        return {
          content: [
            {
              type: "text" as const,
              text: "No roadmap found — create one first with save_roadmap",
            },
          ],
        };
      }
  ...
  },
  {
    ...
    handler: async (args: {
      ...
      if (idx === -1) {
        return {
          content: [{ type: "text" as const, text: `Feature "${args.feature_id}" not found` }],
        };
      }
      // Prevent overwriting the id
      const { id: _id, ...safeUpdates } = args.updates as Record<string, unknown> & {
        id?: unknown;
      };
      ...
      roadmapStore.writeRoadmap(args.project, roadmap);
      return {
        content: [{ type: "text" as const, text: JSON.stringify(roadmap.features[idx], null, 2) }],
      };
    },
  },
  {
