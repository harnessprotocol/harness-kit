...
- Base the roadmap on the existing tasks/epics context above`,
    ...
    if (!parsed.version || !Array.isArray(parsed.phases) || !Array.isArray(parsed.features)) {
      onEvent({
        type: "error",
        message: "Generated roadmap is missing required fields (version, phases, features)",
      });
      return;
    }
  ...
  try {
    ...
    onEvent({
      type: "error",
      message: `Failed to save roadmap: ${err instanceof Error ? err.message : String(err)}`,
    });
    return;
  }
