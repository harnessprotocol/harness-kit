...
export async function planningNode(state) {
    ...
    const response = await model.invoke([
        new HumanMessage(`Based on this implementation spec, create a detailed subtask list.\n\n` +
            `SPEC:\n${state.spec}\n\n` +
    ...
    const createdSubtasks = await Promise.all(subtaskPlans.map(async (s, _i) => {
        const res = await fetch(`${BOARD}/api/v1/projects/${state.projectSlug}/tasks/${state.task.id}/subtasks`, { method: 'POST', headers: { 'Content-Type': 'application/json' },
