...
export async function qaFixingNode(state) {
    ...
    const model = new ChatAnthropic({ ...buildClientOptions(),
        modelName: state.task.default_model ?? 'claude-opus-4-6', maxTokens: 8192 }).bindTools(fsTools);
    const lastQaFeedback = state.messages.at(-1);
    const response = await model.invoke([
        new SystemMessage('You are fixing QA failures. Use the tools to fix failing tests and implementation issues.'),
        new HumanMessage(`Fix the failures identified in QA review.\n\nTask: ${state.task.title}\n\nQA Feedback:\n${lastQaFeedback?.content ?? '(none)'}`),
    ]);
