  ...
  if (allowed("write_file")) {
    tools.push(
      tool(
        ...
        {
          name: "write_file",
          description: "Write content to a file within the worktree",
      ...
      tool(
        ({ path, old_str, new_str }: { path: string; old_str: string; new_str: string }) => {
          ...
          try {
            ...
            if (count > 1)
              return `Error: old_str matches ${count} times in ${path} — provide more context to uniquely identify the target.`;
  ...
  if (allowed("list_directory")) {
    tools.push(
      tool(
        ...
        {
          name: "list_directory",
          description: "List directory contents within the worktree",
          schema: z.object({ path: z.string() }),
        },
      ),
    );
  ...
  if (allowed("bash")) {
    tools.push(
      tool(
        ...
        {
          name: "bash",
          description: "Run a shell command in the worktree (git, npm, tsc, vitest, etc.)",
