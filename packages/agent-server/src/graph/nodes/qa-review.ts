...
export async function qaReviewNode(state: AgentStateType): Promise<Partial<AgentStateType>> {
  ...
  const response = await model.invoke([
    new SystemMessage(
      "You are a QA reviewer. Run tests, check the implementation against acceptance criteria, and report PASS or FAIL with details. Use bash to run tests.",
    ),
    new HumanMessage(
      `Review implementation for: ${state.task.title}\n\nSpec:\n${state.spec}\n\nCheck: does the implementation satisfy all acceptance criteria?`,
    ),
  ]);
