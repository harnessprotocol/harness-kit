...
- **Reconciling a diff, not just closing a gap (AC-11).** Copying onto a surface that already has the resource now shows the per-field deltas it would replace and requires explicit confirmation. Compared on canonical forms, so a skill's file path and a rotated secret are not reported as losses.
