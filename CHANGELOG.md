...
- **Plugin installation (PluginBroker).** `harness-kit sync --only plugin:<name>@<marketplace>` drives each surface's own installer — `claude plugin install`, `codex plugin add`, `copilot plugin install` — with verbs and flags held as registry data read from each tool's `--help`. Two structural guards: argv is never a shell, and no value argument may look like a flag, since a plugin literally named `--config` would otherwise become an option to the installer. Surfaces with no plugin model (pi, opencode) have an unpack driver that places a plugin's skills into their native skills directory and records every path written — built and tested, but with no reachable caller yet, since a surface with no plugin concept never appears as a gap. Reaching it needs an explicit affordance. A missing binary, a non-zero exit and a timeout are three distinct reported outcomes.
- **Recommendations (`status --baseline <path>`).** "You're missing the Postgres MCP server and the research plugin the rest of the team has." Two deterministic sources: machine gaps read from the engine's own gap list, and baseline gaps against a team `harness.yaml`. A baseline is satisfied by the resource existing anywhere on the machine, so the two sources never double-report.
- **Reconciling a diff, not just closing a gap (AC-11).** Copying onto a surface that already has the resource shows the per-field deltas it would replace and requires explicit confirmation. Available from both the CLI (`sync --from <a> --to <b>`) and the drawer; the CLI offers nothing without `--from`, because a diff pair is unordered and picking a side unasked is what the deferral warned against. Compared on canonical forms, so a skill's file path is not reported as a loss.
- **Drift moved into the Machine view (AC-37).** The acknowledge and fix workflow is rendered there rather than reimplemented, `/drift` redirects, and acknowledgements migrate to the shared `~/.harness/harness.db` (additive schema v3). The migration copies and never deletes the source rows.
...
- Plugin cells are actionable but not directly writable **in the app**: the drawer's Apply button is disabled for them, with the reason shown before any click, and the CLI command and agent prompt are offered instead (AC-13). The CLI does install them.
...
- **`harness-kit sync --only <anything>` crashed** with `(filters ?? []).map is not a function`, for every resource kind. Commander treats an option as variadic only when its value name is a plain identifier ending in `...`, and the declaration read `<kind[:name]...>`, so the value arrived as a string. The declaration is fixed and the parser accepts both shapes.
- **Claude Code's third install scope was dropped.** `claude plugin install --scope` takes user/project/local and writes all three verbatim; the reader handled only two and skipped `project` as unrecognized — the scope a team shares through a committed settings file.
