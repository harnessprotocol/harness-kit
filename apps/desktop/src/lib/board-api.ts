...
export const api = {
  projects: {
    ...
    updateSettings: (
      slug: string,
      body: Partial<Pick<Project, "default_harness" | "default_model" | "max_concurrent">>,
    ) =>
      apiFetch<Project>(`/projects/${slug}/settings`, {
        method: "PATCH",
  ...
  tasks: {
    ...
    updateExecution: (
      slug: string,
      taskId: number,
      body: Partial<Omit<TaskExecution, "terminal_id">>,
    ) =>
  ...
  subtasks: {
    ...
    update: (
      ...
      taskId: number,
      subtaskId: number,
      body: Partial<Pick<Subtask, "title" | "description" | "status">>,
    ) =>
