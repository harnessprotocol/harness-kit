...
describe("buildInvokeCommand — { mode: 'skip' }", () => {
  ...
  it("uses --dangerously-skip-permissions", () => {
    expect(buildInvokeCommand("claude", "do something", undefined, cfg)).toBe(
      "claude 'do something' --dangerously-skip-permissions",
    );
  });

  it("includes model", () => {
    expect(buildInvokeCommand("claude", "do it", "claude-opus-4-6", cfg)).toBe(
      "claude 'do it' --dangerously-skip-permissions --model claude-opus-4-6",
    );
  });
});
...
describe("buildInvokeCommand — { mode: 'auto' }", () => {
  ...
  it("uses --permission-mode auto", () => {
    expect(buildInvokeCommand("claude", "do something", undefined, cfg)).toBe(
      "claude 'do something' --permission-mode auto",
    );
  });

  it("includes model", () => {
    expect(buildInvokeCommand("claude", "task", "claude-sonnet-4-6", cfg)).toBe(
      "claude task --permission-mode auto --model claude-sonnet-4-6",
    );
  });

  it("never contains --dangerously-skip-permissions", () => {
    expect(buildInvokeCommand("claude", "task", undefined, cfg)).not.toContain(
...
describe("buildInvokeCommand — { mode: 'allowed-tools' }", () => {
  it("uses --allowedTools with the provided list", () => {
    ...
    expect(buildInvokeCommand("claude", "fix bug", undefined, cfg)).toBe(
      "claude 'fix bug' --allowedTools Read,Grep,Glob",
    );
  });
  ...
  it("uses no permission flags when tools list is empty (Claude prompts for all)", () => {
    ...
    expect(buildInvokeCommand("claude", "task", undefined, cfg)).toBe("claude task");
  });

  it("includes model after the tools flag", () => {
    const cfg: PermissionConfig = { mode: "allowed-tools", tools: ["Read", "Write"] };
    expect(buildInvokeCommand("claude", "task", "claude-opus-4-6", cfg)).toBe(
      "claude task --allowedTools Read,Write --model claude-opus-4-6",
    );
  });
...
describe("buildInvokeCommand — non-Claude harnesses ignore permission config", () => {
  ...
  it("cursor-agent is unaffected by config", () => {
    expect(buildInvokeCommand("cursor-agent", "refactor auth", undefined, skipCfg)).toBe(
      "agent 'refactor auth'",
    );
    expect(buildInvokeCommand("cursor-agent", "refactor auth", undefined, autoCfg)).toBe(
      "agent 'refactor auth'",
    );
  });

  it("copilot is unaffected by config", () => {
    expect(buildInvokeCommand("copilot", "explain this repo", undefined, skipCfg)).toBe(
      "copilot -i 'explain this repo'",
    );
  });

  it("codex is unaffected by config", () => {
    expect(buildInvokeCommand("codex", "add tests", undefined, skipCfg)).toBe("codex 'add tests'");
  });

  it("opencode is unaffected by config", () => {
    expect(buildInvokeCommand("opencode", "add auth", undefined, skipCfg)).toBe(
      "opencode 'add auth'",
    );
  });
});
