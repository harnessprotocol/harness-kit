    ...
    try {
      if (isRunning) {
        if (isAgentTask) {
          ...
          await api.tasks
            .updateExecution(project.slug, task.id, {
              status: "stopped",
            ...
            >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    >
                      ...
                      >
                        {isRunning
                          ? "In Progress"
                          : (COLUMN_META[task.status]?.label ?? task.status)}
                      </span>
                      {/* Subtask count */}
                      {totalSubtasks > 0 && (
              ...
              >
                {(["overview", "subtasks", "logs", "files", "diff"] as TabId[]).map((tab) => {
                  ...
                  if (tab === "subtasks" && totalSubtasks > 0)
                    label = `Subtasks (${totalSubtasks})`;
                  if (tab === "logs" && execStatus && execStatus !== "idle")
                    label = `Logs · ${execStatus}`;
                  if (tab === "files" && fileDiffs.length > 0)
                    label = `Files (${fileDiffs.length})`;
              ...
              <div
                style={{
                  flex: 1,
                  overflowY: activeTab === "logs" && !isAgentTask ? "hidden" : "auto",
                  padding:
                    activeTab === "logs" ||
                    activeTab === "subtasks" ||
                    activeTab === "diff" ||
              ...
              >
                ...
                {activeTab === "overview" && isAgentTask && (
                  <OverviewTab
                    ...
                    currentPhase={agentEventLog.phase}
                        ...
                        >
                          <span style={{ fontSize: 12, color: "var(--text-muted)" }}>
                            ...
                            {task.execution?.started_at &&
                              ` · ${new Date(task.execution.started_at).toLocaleTimeString()}`}
                          </span>
                        </div>
                        <div style={{ fontSize: 13, color: "var(--text-muted)" }}>
                          Terminal output is not preserved after the process ends. Start the task
                          again to see live output.
                        </div>
                      </div>
                    ) : (
                ...
                {activeTab === "files" && (
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    {!task.worktree_path ? (
                      <EmptyState
                        icon="📁"
                        text="No worktree linked. Start the task to create one."
                      />
                    ) : fileDiffs.length === 0 ? (
                ...
                {activeTab === "diff" &&
                  (isAgentTask ? (
                    ...
                    <EmptyState
                      icon="⊟"
                      text="No diff available. This task is not running in agent mode."
                    />
                  ))}

                {/* ─── AGENT FILES TAB (when agent mode) ─── */}
                {activeTab === "files" && isAgentTask && <FilesTab events={agentEventLog.events} />}
              </div>
              ...
              >
                ...
                <button
                  ...
                  onMouseEnter={(e) => {
                    if (!isRunning) (e.currentTarget as HTMLElement).style.color = "#ef4444";
                  }}
                  onMouseLeave={(e) => {
                    (e.currentTarget as HTMLElement).style.color = "var(--text-muted)";
                  }}
