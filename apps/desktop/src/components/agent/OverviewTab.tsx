    ...
    <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 18 }}>
      ...
      <div>
        <div style={{
          ...
          color: '#455270', textTransform: 'uppercase', marginBottom: 10,
        }}>
          Phase Progress
        </div>
        {/* .phase-timeline */}
        <div style={{ display: 'flex', alignItems: 'flex-start' }}>
          {PHASES.map((p, i) => {
            ...
            const circleStyle: React.CSSProperties = {
              ...
              fontSize: 9, zIndex: 1, position: 'relative',
              fontFamily: 'JetBrains Mono, monospace',
              transition: 'all .3s',
              ...(isDone
                ? { background: 'rgba(52,211,153,.2)', border: '1.5px solid #34D399', color: '#34D399' }
                : isActive
                ? { background: 'rgba(75,158,255,.15)', border: '1.5px solid #4B9EFF', color: '#4B9EFF' }
            ...
            return (
              ...
              <div key={p} style={{
                ...
                <div style={{
                  ...
                  textAlign: 'center', color: nameColor, letterSpacing: '.03em',
                }}>
                  {PHASE_LABELS[p]}
                </div>
                ...
                {i < PHASES.length - 1 && (
                  <div style={{
                    ...
                    width: '100%', height: 1, zIndex: 0,
                    background: isDone ? '#34D399' : isActive
      ...
      <div style={{
        background: '#161E2E', border: '1px solid #253352',
        borderRadius: 7, padding: '12px 14px',
      }}>
        <div style={{
          fontSize: 10, fontWeight: 600, letterSpacing: '.08em',
          color: '#455270', textTransform: 'uppercase', marginBottom: 6,
        }}>
      ...
      <div style={{
        ...
        <div style={{
          fontSize: 10, fontWeight: 600, letterSpacing: '.08em',
          color: '#455270', textTransform: 'uppercase', marginBottom: 8,
        }}>
          Steer the agent
        </div>
        {/* .steering-row */}
        <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
          ...
          <textarea
            ...
            onFocus={e => { (e.target as HTMLTextAreaElement).style.borderColor = '#4B9EFF'; }}
            onBlur={e => { (e.target as HTMLTextAreaElement).style.borderColor = '#253352'; }}
          />
          {/* .send-btn */}
          <button
            onClick={handleSteer}
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        ...
        <button
          ...
          style={{
            ...
            transition: 'all .15s', border: '1px solid rgba(167,139,250,.4)',
            background: 'rgba(167,139,250,.15)', color: '#A78BFA',
            letterSpacing: '.01em',
          }}
          onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.background = 'rgba(167,139,250,.2)'; }}
          onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.background = 'rgba(167,139,250,.15)'; }}
        >
          ↩ Take Over
        </button>
        ...
        <button
          ...
          style={{
            padding: '8px 14px', borderRadius: 6, fontSize: 12, fontWeight: 500,
            cursor: 'pointer', fontFamily: 'IBM Plex Sans, sans-serif',
            transition: 'all .15s', border: '1px solid rgba(248,113,113,.25)',
          ...
          onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.background = 'rgba(248,113,113,.1)'; }}
        >
          ✕ Stop
        </button>
      </div>
