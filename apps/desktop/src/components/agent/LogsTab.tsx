...
const S = {
  ...
  // .tool-file-icon
  fileIcon: { opacity: 0.5, fontSize: 12, flexShrink: 0 } as React.CSSProperties,
  // .tool-label
    ...
    <div style={S.body}>
      {entries.map((entry, i) => (
        <div key={i} style={S.entry}>
          {entry.ts && <div style={S.ts}>{new Date(entry.ts).toLocaleString()}</div>}
          {entry.thought && <div style={S.thought}>{entry.thought}</div>}
          {entry.tool &&
            (() => {
