...
export function SessionList({ sessions, currentSessionId, onSelect, onNew, onDelete, onRename, ollamaRunning }: Props) {
    ...
    >
      ...
      >
        <button
          ...
          onMouseEnter={e => { if (ollamaRunning) (e.currentTarget as HTMLElement).style.opacity = '0.85'; }}
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        {sessions.length === 0 ? (
          <div style={{ padding: '20px 12px', textAlign: 'center', fontSize: 12, color: 'var(--fg-subtle)' }}>
            No sessions yet
          </div>
        ) : (
          sessions.map(session => {
            ...
            return (
              <div
                ...
                style={{ flexDirection: 'column', alignItems: 'flex-start', gap: 2, cursor: 'pointer', padding: '8px 12px' }}
      ...
      {contextMenu && (
        ...
        >
          {[
            ...
            {
              label: 'Delete',
              action: () => { onDelete(contextMenu.sessionId); setContextMenu(null); },
              danger: true,
            },
          ].map(item => (
            <button
