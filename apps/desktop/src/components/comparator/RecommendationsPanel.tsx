import type { TaskType, HarnessRecommendation } from "@harness-kit/shared";
...
function WinRateBar({ rate }: { rate: number }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
      ...
      <span style={{ fontSize: "11px", fontWeight: 600, color: "var(--fg-muted)", minWidth: "30px" }}>
        {pct}%
      </span>
    </div>
  );
    ...
    >
      ...
      {loading ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
          {top3.map((rec) => (
            <div key={rec.harnessId} data-testid="recommendation-row" style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
              <div style={{
                display: "flex",
