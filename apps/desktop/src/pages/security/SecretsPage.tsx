...
export default function SecretsPage() {
  ...
  useEffect(() => {
    Promise.all([listRequiredEnv(), readEnvConfig()])
  ...
  async function handleSetSecret() {
    ...
    try {
      ...
      setSecrets((prev) => prev.map((s) => (s.name === modalSecret ? { ...s, isSet: true } : s)));
      setModalSecret(null);
      setSecretValue("");
    } catch (e) {
      setError(String(e));
  ...
  async function handleDeleteSecret(name: string) {
    try {
      ...
      setSecrets((prev) => prev.map((s) => (s.name === name ? { ...s, isSet: false } : s)));
    } catch (e) {
      setError(String(e));
    }
  }
        ...
        {secrets.length === 0 ? (
          ...
          <div>
            ...
            <div
              style={{
                display: "grid",
                gridTemplateColumns: "1fr 2fr 1fr 80px 120px",
                padding: "6px 16px",
        ...
        >
          ...
          >
            ...
            <input
              ...
              autoFocus
              onKeyDown={(e) => {
                if (e.key === "Enter") handleSetSecret();
              }}
              style={{
                width: "100%",
                fontSize: "13px",
            ...
            />
            <div
              style={{ display: "flex", gap: "8px", marginTop: "14px", justifyContent: "flex-end" }}
            >
              <button
                onClick={() => {
                  setModalSecret(null);
