...
export interface UseChatRelayReturn {
  ...
  sendShare: (
    share: Omit<ShareMessage, "id" | "roomCode" | "nickname" | "timestamp" | "type">,
  ) => void;
...
export function useChatRelay(): UseChatRelayReturn {
      ...
      switch (msg.type) {
        ...
        case "room_joined": {
          ...
          void (async () => {
              ...
              localMessages = rows.map((r) => {
                ...
                if (r.msgType === "chat") {
                  ...
                  return {
                    ...
                    event: (r.eventType ??
                      "join") as import("@harness-kit/shared").SystemMessage["event"],
                    detail: r.detail ?? null,
                  };
                }
              });
            ...
            // Save room to SQLite
            chatSaveRoom(roomCode, roomName, myNick, urlToSave).catch((err) =>
              console.warn("[chat] failed to save room:", err),
            );
        ...
        case "message": {
          ...
          chatSaveMessages([row]).catch((err) =>
  ...
  const createRoom = useCallback(
    (nickname: string, name?: string, keepAliveMinutes?: number) => {
      saveNick(nickname);
  ...
  const sendShare = useCallback(
    (shareData: Omit<ShareMessage, "id" | "roomCode" | "nickname" | "timestamp" | "type">) => {
      const current = stateRef.current;
